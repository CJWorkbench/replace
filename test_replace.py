...
def P(colnames: str = '', to_replace: str = '', replace_with: str = '',
      match_case: bool = False, match_entire: bool = False,
...
class TestReplace(unittest.TestCase):
    ...
    def test_str_str_case_sensitive(self):
        table = pd.DataFrame({'A': ['AafredaaA', 'aAafredaAaaa']})
        result = render(table, P(colnames='A', to_replace='a',
                                 match_case=True, regex=False))
        assert_frame_equal(result, pd.DataFrame({'A': ['AfredA', 'AfredA']}))

    def test_match_entire_regex(self):
        table = pd.DataFrame({'A': ['x', 'xa']})
        result = render(table, P(colnames='A', to_replace='[x]*',
                                 replace_with='y', regex=True,
                                 match_entire=True))
    ...
    def test_categorical_rename_one_category(self):
        ...
        result = render(table, P(colnames='A', to_replace='a',
                                 replace_with='d'))
        assert_frame_equal(result, pd.DataFrame({'A': ['d', 'b', 'c', 'b']},
                                                dtype='category'))

    def test_categorical_rename_many_cateogries_to_one(self):
        table = pd.DataFrame({'A': ['a', 'b', 'c', 'b']}, dtype='category')
        result = render(table, P(colnames='A', to_replace='[ac]',
                                 replace_with='d', regex=True))
        assert_frame_equal(result, pd.DataFrame({'A': ['d', 'b', 'd', 'b']},
                                                dtype='category'))
    ...
    def test_categorical_rename_to_existing_category(self):
        ...
        result = render(table, P(colnames='A', to_replace='a',
                                 replace_with='c'))
        assert_frame_equal(result, pd.DataFrame({'A': ['c', 'b', 'c', 'b']},
                                                dtype='category'))
